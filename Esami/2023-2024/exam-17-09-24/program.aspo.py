...
def func3(words, l):
    s = set()
    for w in words:
        for other in words-set([w]):
            if w in other and len(w)>=l:
                s.add(w)
    return s
...
def func4(file_in, file_out, length, chars):
    with open(file_in) as f:
        words = f.read().split()
    words = filter(lambda x: (len(x)>= length and set(x) & set(chars)), words)
...
def follow(img, r, c):
    color = img[r][c]
    w = 0
    while c+w < len(img[0]) and img[r][c+w]==color:
        img[r][c+w] = (0,0,0)
        w+=1
    return w

def draw(img, color, r, c, l):
    for j in range(l):
        img[r][c+j] = color
...
def func5(input_png, output_png, m, M):
    ...
    for r, row in enumerate(img):
        for c, pixel in enumerate(row):
            if pixel != (0,0,0):
                l = follow(img, r, c)
                lines[l] = lines.get(l, []) + [(pixel, r, c, l)]
                # count+=1
                # images.save(img, f'tmp{count:03}.png')
    # print (lines)
    count = 0
    for length, l in lines.items():
        if m<=length<=M:
            for line in l:
                color, r, c, l = line
                draw(img, color, r, c, l)
                count+=1
    images.save(img, output_png)
    return sum(map(len, lines.values())) - count
...
# print(func5('func5/func5_test2.png', 'func5/func5_out2.png', 20, 40))
# print(func5('func5/func5_test3.png', 'func5/func5_out3.png', 40, 100))
# print(func5('func5/func5_test4.png', 'func5/func5_out4.png', 10, 40))
...
def ex1(chars, l):
    ...
    if l == 1:
        return chars
    if l == 2:
        return set(c+c for c in chars)
    pali = set()
    for c in chars:
        partial = ex1(chars, l-2)
        for p in partial:
            pali.add(c+p+c)
    return pali
...
def ex2(root, level=1):
    m1, m2 = root.value, level
    for node in (root.left, root.right):
        if node is not None:
            n1, n2 = ex2(node, level+1)
            if n1*n2 > m1*m2 or (n1*n2 == m1*m2 and m2>n2):
                m1 = n1
                m2 = n2
    return m1, m2
