...
def func1(text):
    dictionary = {k: round(text.count(k) / len(text), 2) for k in set(text) if k.isalpha()}
    return dictionary
    # Your code here
...
def func2(lists: list[list[int]]) -> list[int]:
    occurrence_count = {}
    for sublist in lists:
        for num in set(sublist):
            occurrence_count[num] = occurrence_count.get(num, 0) + 1

    returned_list = [num for sublist in lists for num in sublist if occurrence_count[num] == 1]
    returned_list.sort(reverse=True)
