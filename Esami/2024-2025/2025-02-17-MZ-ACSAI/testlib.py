...
def check_set(res, expected, params=None, expl=''):
    if res != expected:
        if isinstance(res, set):
            my_print(f'''{'*'*50}\n[ERROR] Il set ritornato è sbagliato! / The returned set is incorrect!\nReturned={res}\nexpected={expected}\n{'*'*50}''')
            piu = res - expected
            if piu:
                for extra_key in piu:
                    my_print(f'''\b[ERROR] La valore {extra_key!r} è di troppo.''')
            mancanti = expected - res
            if mancanti:
                for extra_key in mancanti:
                    my_print(f'''\b[ERROR] La valore {extra_key!r} è mancante.''')
        else:
            my_print(f'''{'*'*50}\n[ERROR] Il valore ritornato non è un set! / The returned value is not a set!\nReturned={type(res)!r}\nexpected={type(expected)!r}.{'*'*50}''')
    assert res==expected, '''Il risultato ritornato non è corretto / Incorrect value returned!'''
...
def check_dict(res, expected, params=None, expl=''):
    if res != expected:
        if isinstance(res, dict):
            ...
            for k in expected:
                if k not in res:
                    my_print(f'''\b[ERROR] Manca la chiave {k!r} con valore {expected[k]!r}!''')
                    my_print(f'''\b[ERROR] Key {k!r} with value {expected[k]!r} is missing!''')
                elif expected[k] != res[k]:
                    my_print(f'''\b[ERROR] La chiave {k!r} dovrebbe avere il valore {expected[k]!r} invece che {res[k]!r}''')
                    my_print(f'''\b[ERROR] Key {k!r} should have the value {expected[k]!r} instead of {res[k]!r}''')
            diff = set(res.keys()) - set(expected.keys())
            if diff:
                for extra_key in diff:
                    my_print(f'''\b[ERROR] La chiave {extra_key!r} è di troppo.''')
                    my_print(f'''\b[ERROR] La key {extra_key!r} is unexpected.''')
        else:
...
def check_exam_constraints():
    ...
    constraint5 = all((constraint3, constraint4))
    if not constraint4:
