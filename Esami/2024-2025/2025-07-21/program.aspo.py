...
def func1(embedding_list1, embedding_list2):
    ...
    d = [(w1[0], w2[0], cosine_similarity(w1[1], w2[1])) for w1 in embedding_list1 for w2 in embedding_list2]
    return min(d, key = lambda x: (-x[2], x[0], x[1]))
    pass
embeddings1 = [('king', [0.1, 0.2]), ('queen', [0.3, 0.4])]
embeddings2 = [('man', [0.05, 0.15]), ('woman', [0.25, 0.35])]
# For simplicity, if (queen, woman) has highest similarity.
...
Example:
  text_corpus = ["This is a great movie!", "It was terrible.", "A perfect waste of time."]
  sentiment_lexicon = {
      'great': 0.8,
      'terrible': -0.9,
      'perfect': 0.7,
  ...
  func2(text_corpus, sentiment_lexicon, 2) should return (order of sentences might vary for same score):
  {
...
def tokenize_alphabetic(text):
    ...
    """
    return re.findall(r'[a-z]+', text, flags=re.I)

from collections import defaultdict

def func2(text_corpus, sentiment_lexicon, top_k_sentences):
    d = dict.fromkeys(text_corpus, 0)
    for sentence in text_corpus:
        for word in tokenize_alphabetic(sentence):
            d[sentence] += sentiment_lexicon.get(word.lower(), 0)
    sentences = sorted([(v, k) for k,v in d.items() if v>0], reverse=True)
    return {'overall_sentiment': round(sum(d.values())/len(d), 2),
            'top_positive_sentences': sentences[:top_k_sentences],
            'total_sentences_analyzed': len(d)}
...
def func3(sentence_list, forbidden_words):
    ...
    for i in range(len(sentence_list)-1, -1, -1):
        ...
        if len(set(map(str.lower, tokenize_alphabetic(sentence))) & forbidden_words) >0:
            sentence_list.pop(i)
    return tot - len(sentence_list)
...
import collections
def func4(text, n):
    token = list(map(str.lower, tokenize_alphabetic(text)))
    d = [(w.lower(), token.count(w.lower())) for w in set(token)]
    return sorted(d, key = lambda x: (-x[1], x[0]))[:n]
    pass
text = "The quick brown fox jumps over the lazy dog. The fox is quick."
print(func4(text, 2))
print(func4(text, 3))
...
'''
def func5(word_list):
    d = {"".join(sorted(w.lower())):set() for w in word_list}
    for word in word_list:
        d["".join(sorted(word.lower()))].add(word)
    return d
...
def ex1(root, result, target_value):
    res = ex1_(root, [], target_value)
    if len(res) > 0:
        result[:] = max(res, key = len)
    
def ex1_(root, path, target_value):
    if root is None:
        return []
    res = []
    if root.value == target_value:
        res.append(path+[root.value])
    res.extend(ex1_(root.left, path+[root.value], target_value))
    res.extend(ex1_(root.right, path+[root.value], target_value))
    return res
...
You can use the load and save functions from the images.py module.
'''

from images import load, save

def ex2(in_path, out_fpath, y, x, replacement_color):
    im = load(in_path)
    c = im[x][y]
    im = ex2_(im, x, y, c, replacement_color)
    save(im, out_fpath)
        
    
def ex2_(im, X, Y, c, newc):
    for x,y in ((X+1, Y), (X-1, Y), (X, Y-1), (X, Y+1)):
        try:
            if im[x][y] == c:
                im[x][y] = newc
                im = ex2_(im, x, y, c, newc)
        except IndexError:
            pass
    return im
