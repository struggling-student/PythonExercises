...
def check_list(res, expected, params=None, expl=''):
    if res != expected:
        if isinstance(res, list):
            ...
            if diff>=0:
                expected_padded = expected + [""]*diff # Use a new variable to not modify original expected
                zz = zip(res, expected_padded)
                print('{:<30}   {:>30}'.format('returned', 'expected'))
            else:
                res_padded = res+[""]*(-diff) # Use a new variable to not modify original res
                zz = zip(expected, res_padded)
                print('{:<30}   {:>30}'.format('expected', 'returned'))
            for i, j in zz:
                # Convert i and j to string before formatting
                if i!=j:
