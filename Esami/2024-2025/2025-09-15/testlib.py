    ...
    if res != expected:
        if isinstance(res, list):
            ...
            if diff>=0:
                ...
                zz = zip(res, expected)
                print('{:<40}   {:>40}'.format('returned', 'expected'))
            else:
                res = res+[""]*(-diff)
                zz = zip(expected, res)
                print('{:<40}   {:>40}'.format('expected', 'returned'))
            for i, j in zz:
                if i!=j:
                    print(f'\b[ERROR]{str(i):<33}   {str(j):>40}')
                else:
                    print(f'{str(i):<40}   {str(j):>40}')
        else:
...
def check_exam_constraints():
    ...
    constraint5 = all((constraint1, constraint3, constraint4))
    if not constraint4:
