    ...
    if res != expected:
        if isinstance(res, dict):
            ...
            for k in expected:
                if k not in res:
                    print(
                        f'''\b[ERROR] Manca la chiave {k!r} con valore {expected[k]!r}!''')
                    print(
                        f'''\b[ERROR] Key {k!r} with value {expected[k]!r} is missing!''')
                elif expected[k] != res[k]:
                    print(
                        f'''\b[ERROR] La chiave {k!r} dovrebbe avere il valore {expected[k]!r} invece che {res[k]!r}''')
                    print(
                        f'''\b[ERROR] Key {k!r} should have the value {expected[k]!r} instead of {res[k]!r}''')
...
def check_exam_constraints():
    ...
    constraint1 = len([name for name, grade in grades.items() if
                       grade > 0 and name.startswith('func')]) >= 3
    constraint2 = len([name for name, grade in grades.items() if
                       grade > 0 and name.startswith('ex')]) >= 1
    ...
    if not constraint4:
        print(
            f'YOU HAVE NOT FILLED YOUR PERSONAL DETAILS: {COL["RED"]}EXAM NOT PASSED{COL["RST"]}')
    ...
    BOLD = '\033[1m'
    print(
        f"Personal info present:       {COL['BOLD']} {COL['GREEN'] if constraint4 else COL['RED']} {constraint4}{COL['RST']}{COL['ENDC']}")
    print(
    ...
    print(
        f"Exam passed:                 {COL['BOLD']} {COL['GREEN'] if constraint5 else COL['RED']} {constraint5}{COL['RST']}{COL['ENDC']}")
