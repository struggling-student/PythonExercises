...
from string_list all the strings that contain the 2-characters string 
built by concatenating the first and last letter of 'word'.
...
Example:
    ...
    indeed 'assenzio', 'Minosse' are deleted because they contain 'se'
'''
...
'number' as the value but as an float type.  In addition, a student_id can be
associated with more than one number: in case this happens, the dictionary
needs to contain the average of all numbers found for that student_id.
Example:
Content of func2_test_1.txt
 ...
 26,123456
 91,111111
The function func2('func2_test_1') 
returns {'123456': 26.5, '121212': 78.5, '111111': 90.5}
'''
...
The strings are written in order of:
- increasing order by the number of characters.
- In case of a tie, strings are sorted in reverse alphabetical order 
  not considering the difference between lowercase and uppercase.
- in case of a tie, strings are sorted in reverse alphabetical order 
...
Then, the function computes the frequency of the words found and has
...
- the words appear in reverse alphabetical order
- every word appears preceded by enough spaces to align all words to the right,
  followed by one space, many asterisks ('*') as its frequency, and one newline character ('\n')
- the number of spaces added before is such that all the asterisks are
  left-aligned in all the lines.

The inital part of the string hence will be, once printed:
      una *
 topolino **
     sono *
       si *
...
NOTICE: the longest word is 'purtroppo', and has no spaces on the left.
...
'img', an image modeled as a list of lists of pixels and effects a LEFT rotation
of the image by 90 degrees, followed by a reflection respect to its vertical axis. 
The new rotated image must be saved in 'output_file_name' via the images module.
The function returns a tuple where the first element is the height and
the second is the width of the rotated-reflected image.

Example
image     rotation           reflection
abc         -->         cf      -->     fc
def                     be              eb
                        ad              da
...
"""
Ex1: 7 points
Implement a recursive function ex1(root), or tht uses another recursive function, 
that takes a string pointing to a directory as input and
recursively explores the directory tree and returns a dictionary.
...
integer numbers separated by spaces.
...
reverse alphabetical order of the file names .txt.

If the directory does not contain any .txt files then that directory
does not appear in the dictionary.

If the function is called on 'ex1/A', it will return the following dictionary:
...
NOTE: Do NOT define the recursive function as an inner function inside ex2
else your exercise will not pass the recursion test.
...
recursion, which takes as input a list of positive integers 'nums' and a list
of strings 'ops' indicating operations on the numbers.
Both lists could contain duplicates.
...
two or more numbers taken from 'nums' with operations from the 'ops'.
...
1. Once a number is used in the expression, it can no longer be used
   (unless there are other duplicates)
   For example, if nums=[5,5,0] and ops=['+','*']
   '5+5+0' is a valid expression, while '0+5+0' is NOT valid.
   (too many 0)

2. Operations cannot be reused.
   (unless there are other duplicates)

   For example, '5+5+0' is not valid because '+' was used twice.

The function returns a set with all the generated expressions
with maximum lenght. (i.e. that contains all numbers or all operators)

Example: if nums = [5, 0, 5] and ops = ['+', '*', '+'], the function will generate:
{'0+5+5', '5*5+0', '0*5+5', '5+5*0', '5+0*5', '5*0+5', '5+0+5', '5+5+0', '0+5*5'} 

NOTE: do NOT define the recursive function as an inner functon of ex2 otherwise
your exercise will fail the recursive test.
NOTE: we strongly suggest dividing the exercise into subproblems
organizing the code into small functions for each subproblem.
"""
