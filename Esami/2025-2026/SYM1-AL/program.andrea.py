...
def func2(D : dict[int, list[str]]) -> list[str]:
    # scrivi qui il tuo codice
    pass
    return [ (sorted(v, reverse=True)[k],sorted(v, reverse=True)[-k-1]) for k,v in D.items()]
...
def func3(strList : list[str]) -> list[int]:
    ...
    return [sum(ord(c) for c in s) for s in sorted(strList, key=criterio)]
...
def func4(string_list1 : list[str], string_list2 : list[str]) -> list[str]:
    ...
    pass
    parole1 = [p for p in string_list1 if any(s in p for s in string_list2)]
    parole2 = [p for p in string_list2 if any(s in p for s in string_list1)]
