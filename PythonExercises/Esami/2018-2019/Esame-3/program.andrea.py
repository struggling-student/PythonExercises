...
def es2(mat):
    ...
    h = len(mat)
    interno = set()
    bordo   = set(mat[0]) | set(mat[-1])
    for line in mat[1:-1]:
        interno |= set(line[1:-1])
        bordo.add(line[0])
        bordo.add(line[-1])
    L1 = sorted( interno - bordo )
    L2 = sorted( bordo - interno )
    return L1, L2
...
def es4(tree):
    # inserisci qui il tuo codice
    return sorted(livelli_con_foglie(tree, 0))

def livelli_con_foglie(radice, livello):
    if not radice.f:
        return {livello}
    else:
        lcf = set()
        for s in radice.f:
            lcf |= livelli_con_foglie(s, livello+1)
        return lcf
...
def minimo(tree):
    path = [tree.id]
    if tree.f:
        percorsi = [ minimo(s) for s in tree.f ]
        ms = min(percorsi, key=lambda p: p[0])
        if ms[0] < path[0]:
            path = ms + path
    return path
...
def es6(fimm, x, y):
    ...
    h = len(img)
    visitati = { (x,y) }
    nn = new_neighbor(img, x, y, w, h, visitati)
    while nn:
        X, Y = nn
        visitati.add(nn)
        nn = new_neighbor(img, X, Y, w, h, visitati)
...
def new_neighbor(img, x, y, w, h, visitati):
    for dx,dy in [ (-1,0), (1, 0), (0, -1), (0, 1) ]:
        X = x+dx
        Y = y+dy
        if  0 <= X < w and \
            0 <= Y < h and \
            img[Y][X] == (255,255,255) and \
            (X,Y) not in visitati:
                return X, Y
    return None
...
def es7(stringa1,stringa2,k):
    # inserisci qui il tuo codice
    S1 = { stringa1[i:i+k] for i in range(len(stringa1)-k+1) }
    S2 = { stringa2[i:i+k] for i in range(len(stringa2)-k+1) }
    common = S1 & S2
    return sorted( common, key=lambda s: (nvoc(s), s) )

def nvoc(S):
    chars = set(S)
    vocali = chars & set('aeiou')
    return len(vocali)
