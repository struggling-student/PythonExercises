...
es1(lista) restituisce 8 e la lista risulata modificata in [5,2,3,1]
...
Il primo elemento della coppia e' la lista di tutti gli interi  presenti all'interno 
...
def es2(mat):
	...
	for i in range(len(mat)):
	    for j in range(len(mat[0])):
	        if i not in {0,len(mat)-1} and j not in {0,len(mat[0])-1}:
	            interno.add(mat[i][j])
	        else:
	            orlo.add(mat[i][j])
	return (sorted(interno-orlo), sorted(orlo-interno))
...
def es3(ftesto):
    ...
    for comando in lista:
        dir,val=comando[0],int(comando[1:])
        if dir=='N': 
...
def es4(tree):
    # inserisci qui il tuo codice
    return sorted( es4b(tree,0))
    
def es4b(tree,k):
    # restituisce l'insieme coi livelli in cui sono presenti foglie  
    if tree.f==[]: return {k}
    ins=set()
    for x in tree.f:
        set1= es4b(x,k+1)
...
def es6(fimm, w1, h1):
    ...
    tot=1
    while True:
        if w1+1<w and (w1+1,h1) not in ins and img[h1][w1+1]==(255,255,255):
            w1=w1+1
            ins.add((w1,h1))
            tot+=1
        elif  h1+1<h and (w1,h1+1) not in ins and img[h1+1][w1]==(255,255,255):
            h1=h1+1
            ins.add((w1,h1))
            tot+=1
        elif  0<=w1-1 and (w1-1,h1) not in ins and img[h1][w1-1]==(255,255,255):
            w1=w1-1
            ins.add((w1,h1))
            tot+=1
        elif  0<=h1-1 and (w1,h1-1) not in ins and img[h1-1][w1]==(255,255,255):
            h1=h1-1
            ins.add((w1,h1))
            tot+=1
        else: return tot
...
def es7(stringa1,stringa2,k):
    # inserisci qui il tuo codice
    ins=set()
    for i in range(len(stringa1)-k+1): 
        if stringa1[i:i+k] in stringa2 : ins.add(stringa1[i:i+k])
    lista=[]
    for x in ins:
        c=0
        if 'a' in x: c+=1
