...
def ex1(triangles):
    ...
    N = len(triangles)
    # raccolgo tutti i triangoli che sono triangoli rettangoli
    triangoli = [ t for t in triangles if isrectangle(t)]
    triangles[:] = triangoli   # e sostituisco il contenuto con l'elenco di triengoli giusti
    return N - len(triangles)  # la differenza è il numero di triangoli eliminati

def isrectangle(t):
    cat1, cat2, ipo = sorted(t)   # estraggo l'ipotenusa che è sempre il più grosso
    return round(cat1**2+cat2**2, 3) == round(ipo**2, 3)    # e controllo
...
def ex2(img_in, img_out, colors):
    ...
    # raccolgo le coordinate ed il colore dei pixel che sono
    punti = [ (x,y,pixel)   for y,riga in enumerate(img) 
                            for x,pixel in enumerate(riga) 
                            if  pixel in colors         # tra i colori elencati
                                and pixel != (0,0,0)    # non di sfondo 
            ]
    # disegno tutti i rettangoli nell'ordine
    for x, y, pixel in punti:
        h,w = colors[pixel]
        draw_rectangle(img, x, y, w, h, pixel)
    # salvo l'immagine
    images.save(img,img_out)
    # e torno il numero di rettangoli disegnati
    return len(punti)

def draw_rectangle(img, x, y, w, h, pixel):
...
def ex3(directory, namefile):
    ...
    pass
    # cerco i file
    files = cercafiles(directory, namefile)
    # ne estraggo le liste di interi
    liste = [ estrailistaint(file) for file in files ]
    # e creo la lista di somme
    somme = [ sum(lista) for lista in zip(*liste) ]
    return somme

def cercafiles(directory, namefile):
    trovati = []
    for nome in os.listdir(directory):
        fullname = directory + '/' + nome
        if os.path.isdir(fullname):
            trovati += cercafiles(fullname, namefile)
        elif nome == namefile:
            trovati.append(fullname)
    return trovati

def estrailistaint(filename):
...
def ex4(strings, n):
    # WRITE HERE YOUR CODE
    pass
    # torno la lista ordinata per lunghezza decrescente e parola crescente
    return sorted(set(_ex4(strings,n)), key=lambda s: (-len(s),s))

def _ex4(strings, n):
    if n == 1:              # caso base, se N==1 
        return strings      # posso creare solo le stesse stringhe
    else:
        # altrimenti concateno davanti ciascuna delle stringhe
        # a tutte le stringhe ottenute
        # ignorando quella stringa
        return [ s+l for s in strings for l in _ex4(strings-{s}, n-1)]
